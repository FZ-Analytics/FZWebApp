...
<%!
    String getData(HttpServletRequest request) {
        ...
        try {
            ...
            String sql = 
                    "select b.VehicleName name, a.latitude, a.longitude"
                    + ", date_format(a.endDate,'%H:%i') tm"
                    + " from fbGpsTrackersim a "
                    + "left join fbvehicle b on a.VehicleID=b.VehicleID"
                    //+ " where str_to_date(endDate,'%Y-%m-%d') = '" + dt + "'"
                    + " order by b.VehicleName, date_format(a.endDate,'%H:%i')"
                    ;
